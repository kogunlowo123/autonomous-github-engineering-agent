...
def configure_logging(level: str = "INFO", *, json_output: bool = True) -> None:
    """Configure the ``ghagent`` logger tree.
    ...
    """
    logger = logging.getLogger("ghagent")
...
def get_logger(name: str) -> logging.Logger:
    """Return a logger namespaced under ``ghagent``."""
    return logging.getLogger(name if name.startswith("ghagent") else f"ghagent.{name}")
